...
let rec dump_expr (depth : int) (e : expr) : string =
  ...
  | BinOp (op, e1, e2) ->
    Printf.sprintf "%sBinOp(%s)\n%s\n%s" pad (string_of_op op)
      (dump_expr (depth + 1) e1)
      (dump_expr (depth + 1) e2)
  | UnaryOp (op, e1) ->
    Printf.sprintf "%sUnaryOp(%s)\n%s" pad (string_of_uop op)
      (dump_expr (depth + 1) e1)
  | Call (name, args) ->
    let args_dumped = List.map (dump_expr (depth + 1)) args in
...
and dump_stmt (depth : int) (s : stmt) : string =
  ...
  | Print exprs ->
    let dumped = List.map (dump_expr (depth + 1)) exprs in
...
let rec expr_to_string (e : expr) : string =
  ...
  | Call (name, args) ->
    let args_s = List.map expr_to_string args in
...
and pp_stmt (depth : int) (s : stmt) : string =
  ...
  | Print exprs ->
    let args_s = List.map expr_to_string exprs in
    Printf.sprintf "%sprint(%s);" pad (String.concat ", " args_s)
  | Block stmts ->
    Printf.sprintf "%s{\n%s\n%s}" pad
      (pp_stmts (depth + 1) stmts) pad
